...
export default function MapScreen() {
  ...
  // простое кластерайзинг при далёком масштабе
  const clusters = useMemo<Cluster[]>(() => {
    ...
    for (const s of spots) {
      ...
      if (c) {
        ...
        map.set(key, { key, lat: s.lat, lng: s.lng, count: 1 });
      }
    }
    ...
    <View style={{ flex: 1, backgroundColor: '#0A0A0A' }}>
      ...
      {showSpots && !showIndividualSpots && (
        <View style={styles.hint}>
          <Text style={styles.hintText}>{t('map.zoomInToSee')}</Text>
        </View>
      )}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={styles.spotRow}>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      // здесь можно прокинуть ref карты и центрировать камеру; сейчас просто закрываем список
                      setListOpen(false);
                    }}
                    style={styles.goBtn}
                  >
